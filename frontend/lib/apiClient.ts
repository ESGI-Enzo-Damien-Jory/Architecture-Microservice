    ...
    this.axiosInstance.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        if (error.response?.status === 401 && originalRequest && !originalRequest._retry) {
          ...
          try {
            const { useAuth } = require("@/lib/authStore");
            console.log("[API_CLIENT] Access token expired, attempting refresh");
            ...
            if (success) {
              ...
              console.log("[API_CLIENT] Token refresh failed, redirecting to login");
        ...
        console.error(`[API_CLIENT] ${error.response?.status || 'Network'} error:`, {
